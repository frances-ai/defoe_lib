...
from s3_service import read_data_in_s3
...
def do_query(df, config=None, logger=None, context=None):
    ...
    if collection == NLSCollection.EB.value:
        ...
        if start_year and end_year:
            newdf = fdf.filter(fdf.description.isNotNull()).filter(fdf.year >= start_year).filter(
                fdf.year <= end_year).select(fdf.year, fdf.term_uri, fdf.description, fdf.term_name, fdf.edition_title, fdf.edition_uri,
            ...
            newdf = fdf.filter(fdf.description.isNotNull()).filter(fdf.year >= start_year).select(fdf.year, fdf.term_uri, fdf.description, fdf.term_name, fdf.edition_title, fdf.edition_uri,
                                             fdf.edition_number, fdf.source_file_uri, fdf.source_file_path,
                                             fdf.volume_uri, fdf.volume_number, fdf.start_page_uri,
                                             fdf.start_page_number)
        elif end_year:
            newdf = fdf.filter(fdf.description.isNotNull()).filter(fdf.year <= end_year).select(fdf.year, fdf.term_uri, fdf.description, fdf.term_name, fdf.edition_title, fdf.edition_uri,
                                             fdf.edition_number, fdf.source_file_uri, fdf.source_file_path,
                                             fdf.volume_uri, fdf.volume_number, fdf.start_page_uri,
                                             fdf.start_page_number)
        else:
            newdf = fdf.filter(fdf.description.isNotNull()).select(fdf.year, fdf.term_uri, fdf.description, fdf.term_name, fdf.edition_title, fdf.edition_uri,
                                             fdf.edition_number, fdf.source_file_uri, fdf.source_file_path,
                                             fdf.volume_uri, fdf.volume_number, fdf.start_page_uri,
                                             fdf.start_page_number)
    else:
        ...
        if start_year and end_year:
            newdf = fdf.filter(fdf.description.isNotNull()).filter(fdf.year >= start_year).filter(
                fdf.year <= end_year).select(fdf.year, fdf.page_uri, fdf.description, fdf.page_number, fdf.series_title, fdf.series_uri,
                                             fdf.series_number, fdf.source_file_uri, fdf.source_file_path,
                                             fdf.volume_uri, fdf.volume_title, fdf.volume_number)
        elif start_year:
            newdf = fdf.filter(fdf.description.isNotNull()).filter(fdf.year >= start_year).select(fdf.year, fdf.page_uri, fdf.description, fdf.page_number, fdf.series_title, fdf.series_uri,
                                             fdf.series_number, fdf.source_file_uri, fdf.source_file_path,
                                             fdf.volume_uri, fdf.volume_title, fdf.volume_number)
        elif end_year:
            newdf = fdf.filter(fdf.description.isNotNull()).filter(fdf.year <= end_year).select(fdf.year, fdf.page_uri,fdf.description, fdf.page_number, fdf.series_title, fdf.series_uri,
                                             fdf.series_number, fdf.source_file_uri, fdf.source_file_path,
                                             fdf.volume_uri,fdf.volume_title, fdf.volume_number)
        else:
            newdf = fdf.filter(fdf.description.isNotNull()).select(fdf.year, fdf.page_uri, fdf.description, fdf.page_number, fdf.series_title, fdf.series_uri,
                                             fdf.series_number, fdf.source_file_uri, fdf.source_file_path,
                                             fdf.volume_uri, fdf.volume_title, fdf.volume_number)
    ...
    if collection == NLSCollection.EB.value:
        ...
        preprocess_articles = articles.flatMap(
            lambda t_articles: [(t_articles[0], t_articles[1], preprocess_clean_page(t_articles[2], preprocess_type), t_articles[3], t_articles[4], t_articles[5],
                                        t_articles[6], t_articles[7], t_articles[8], t_articles[9], t_articles[10], t_articles[11], t_articles[12])])
    else:
       #(year-0, page_uri-1, description-2, page_number-3, series_title-4, series_uri-5, series_number-6, source_file_uri-7, source_file_path-8, volume_uri-9, volume_title-10, volume_number-11)
        preprocess_articles = articles.flatMap(
            lambda t_articles: [(t_articles[0], t_articles[1], preprocess_clean_page(t_articles[2], preprocess_type), t_articles[3], t_articles[4], t_articles[5],
                                        t_articles[6], t_articles[7], t_articles[8], t_articles[9], t_articles[10], t_articles[11])])


    if data_file:
        keysentences = []
        data_stream = read_data_in_s3(data_file).decode("utf-8")

        for keysentence in data_stream.split("\n"):
    ...
    if target_sentences:
        ...
        if target_filter == "or":
            ...
            target_articles = reduce(lambda r, target_s: r.filter(lambda year_page: target_s in year_page[2]), clean_target_sentences, target_articles)
    else:
        target_articles = preprocess_articles
        ...
        maching_idx = filter_articles.map(
            lambda year_page: (
                (year_page[0],
                 ...
                 year_page[11],
                 year_page[12],
                 get_text_keysentence_idx(year_page[2],
                                          keysentences)
                 )
            ))
        #(year-0, term_uri-1, description-2, term_name-3, edition_title-4, edition_uri-5, edition_number-6, source_file_uri-7, source_file_path-8, volume_uri-9, volume_number-10, start_page_uri-11, start_page_number-12)
            ...
            lambda sentence_data: [
                (sentence_data[0],
                 {"term_uri": sentence_data[1],
                  ...
                  "snippet": get_concordance_string(sentence_data[2], word_idx[0], word_idx[1], window)}) \
                for word_idx in sentence_data[13]])
    else:
        maching_idx = filter_articles.map(
            lambda year_page: (
                (year_page[0],
